...
enum abstract EventType(Int) from Int to Int
{
	...
	**/
	...
	/**
	* An event determining a gamepad connecting to the device.
	**/
	var EVENT_GAMEPAD_CONNECTED 	=   25;
	/**
	* An event determining a gamepad disconnecting from the device.
	**/
	var EVENT_GAMEPAD_DISCONNECTED	=	26;
	/**
	* Occurs when the mouse lock mechanism changes. This has no data associated with it.
	**/
	var EVENT_MOUSE_LOCK_CHANGE		=	27;
	/**
	* Occurs if an error is produced by toggling mouse lock. This has no data associated with it.
	**/
	var EVENT_MOUSE_LOCK_ERROR		=	28;
	/**
	* Occurs when the accelerometer on a compatible device detects motion of the physical device.
	**/
	var EVENT_ACCELEROMETER			=	29;
	/**
	* Occurs when the gyroscope on a compatible device detects rotational changes in the device.
	**/
	var EVENT_GYROSCOPE				=	30;
}